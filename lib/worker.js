...
class Worker {
    ...
    async process_message(message) {
        ...
        let leader_object = shared_object[this.channel_name];
        ...
        try {
            ...
            this.processing_messages = this.processing_messages.filter((_message) => _message.message_id != message.message_id);
        }
        ...
        while (!this.is_stopped) {
            ...
            if (leader_inerface != undefined && leader_inerface.is_init) {
                if (leader_inerface.heartbeat[this.id] == undefined) {
                    ...
                    leader_inerface.in_process_messages[this.id] = this.processing_messages;
                }
                ...
                while ((message = leader_inerface.ready_messages[this.id].shift()) !=
                    ...
                    this.queued_messages = this.queued_messages.filter((_message) => _message.message_id != removed_id);
